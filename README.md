...
* **`multivariable_logistic_regression`**: Runs the main figures and analyses evaluating sex, epilepsy type, and age.
* **`mann_whitney_tests`**: Runs the Mann-Whitney U tests and generates the associated figures.
* **`flowchart`**: Generates the supplemental figure of the patient cohort attrition and selection.
* **`likelihood_ratio_testing`**: Runs the testing between the interaction terms of the main regression model.
* **`multivariable_logistic_regression_menopause`**: Runs the main analyses with the age split defined at 51 years old.
* **`multivariable_logistic_regression_menopause_interaction`**: Runs the analyses with interaction terms utilizing the age split of 51 years old.
