...
def build_patient_seizure_metrics(vuniq: pd.DataFrame) -> pd.DataFrame:
    ...
    mean_sz  = grp["sz_freqs"].apply(lambda x: np.nanmean(x[np.isfinite(x)]) if np.isfinite(x).any() else np.nan)
...
def fit_mixed_effects_models(pair_table, n_boot=0, alpha=0.05):
    ...
    T = pair_table.loc[keep].copy().reset_index(drop=True)

    T["HasSz_bin"] = (T["HasSz"] == 1).astype(int)
    T["AbsLag_years"] = np.abs(T["SignedLag_years"])
    T["VisitAfterEEG"] = (T["SignedLag_years"] >= 0).astype(int)

    T["PatientID"] = T["PatientID"].astype(str)

    T["EpiType3"] = pd.Categorical(
        T["EpiType3"],
        categories=["Temporal", "Frontal", "General"]
    )

    print(
        f"[Model table] {len(T)} pairs, "
        f"{T['PatientID'].nunique()} patients"
    )
    ...
    def extract_fe(mdl):
        ...
        names = mdl.model.exog_names

        beta = np.asarray(mdl.fe_mean)
        se = np.asarray(mdl.fe_sd)
    ...
    fe_M1 = extract_fe(mdl_M1)
    fe_M2 = extract_fe(mdl_M2)
    ...
    fe_before = extract_fe(
        mdl_before
    )

    if fe_after is not None:
        print("\nAFTER model")
        print(fe_after)

    if fe_before is not None:
        print("\nBEFORE model")
        print(fe_before)
    ...
    mmr = {
        ...
        "BootstrapTable1": None,
        "BootstrapBetas1": None,

        "BootstrapTable2": None,
        "BootstrapBetas2": None,

        "BootstrapConvergence": None,
    ...
    print(
        "\nDone. "
        "Primary model: BinomialBayesMixedGLM "
        "with patient random intercept."
    )
...
def compute_visit_eeg_gaps(vuniq, report_kept):
    ...
    # -----------------------------
    # 1. Parse EEG lists correctly
    # -----------------------------
    eeg_raw = report_kept["start_time_deid"].astype(str).str.strip()

    # convert stringified lists -> real lists
    
    print(eeg_raw)
    
    def safe_parse_eeg(x):
        if pd.isna(x):
            return []
        ...
        # Case 2: single timestamp (bad formatting but still usable)
        return [x]
    
    eeg_lists = eeg_raw.apply(safe_parse_eeg)
    print(eeg_lists)
    ...
    # explode into one row per EEG date
    eeg_tbl = eeg_tbl.explode("EEG_List")

    eeg_tbl["EEG_Date"] = pd.to_datetime(
        eeg_tbl["EEG_List"],
        errors="coerce"
    )

    eeg_tbl = eeg_tbl.dropna(subset=["EEG_Date"])

    # group EEGs by patient (FAST lookup)
    eeg_map = eeg_tbl.groupby("Patient")["EEG_Date"].apply(list).to_dict()

    # -----------------------------
    # 2. Prepare visit table
    # -----------------------------
    gap_tbl = vuniq[["Patient", "VisitDate"]].copy()

    gap_tbl["VisitDate"] = pd.to_datetime(
        gap_tbl["VisitDate"],
        errors="coerce"
    )
    ...
    # -----------------------------
    # 3. Compute gaps safely
    # -----------------------------
    for p, grp in gap_tbl.groupby("Patient"):

        eeg_dates = eeg_map.get(p, None)
        if eeg_dates is None or len(eeg_dates) == 0:
            continue

        eeg_dates = np.array(eeg_dates, dtype="datetime64[ns]")

        for idx in grp.index:

            vd = gap_tbl.at[idx, "VisitDate"]
            ...
            gap_tbl.at[idx, "MinAbsGap_days"] = (
                diffs.min() / np.timedelta64(1, "D")
            )

    # -----------------------------
    # 4. Derived column (OUTSIDE LOOP)
    # -----------------------------
...
def restrict_visits_by_min_abs_gap(vuniq, report_kept, min_days, max_days):
    ...
    # =========================================================
    # 1. CLEAN + PARSE EEG DATA (handles BOTH formats)
    # =========================================================

    def safe_parse_eeg(x):
        """Handles:
        - stringified list: ["2000-01-01", ...]
        - single timestamp: 7/10/03 14:08
        """
        if pd.isna(x):
            return []
    ...
    eeg_lists = report_kept["start_time_deid"].apply(safe_parse_eeg)
    ...
    # robust datetime parsing (handles both formats in your data)
    ...
    # =========================================================
    # 2. PRE-GROUP EEGs (FAST LOOKUP)
    # =========================================================
    eeg_map = eeg_tbl.groupby("Patient")["EEG_Date"].apply(
        lambda x: np.array(x, dtype="datetime64[ns]")
    ).to_dict()
    ...
    gap_tbl = gap_tbl.dropna(subset=["VisitDate"])

    gap_tbl["MinAbsGap_days"] = np.nan

    # =========================================================
    # 4. COMPUTE MIN ABS GAP (SAFE)
    # =========================================================
    for p, grp in gap_tbl.groupby("Patient"):
        ...
        for idx in grp.index:
            ...
            vd64 = np.datetime64(vd)

            diffs = np.abs(eeg_dates - vd64)
    ...
    # =========================================================
    # 5. DROP INVALID RESULTS (CRITICAL FOR STABILITY)
    # =========================================================
    gap_tbl = gap_tbl.dropna(subset=["MinAbsGap_days"])

    # safety check (prevents silent bootstrap collapse)
    if len(gap_tbl) == 0:
        raise ValueError("All MinAbsGap_days are NaN — check datetime parsing.")

    # =========================================================
    # 6. APPLY FILTER
    # =========================================================
    keep_mask = (
        (gap_tbl["MinAbsGap_days"] >= min_days) &
        (gap_tbl["MinAbsGap_days"] <= max_days)
    )

    result = gap_tbl[keep_mask].copy()

    # =========================================================
    # 7. DEBUG OUTPUT (IMPORTANT)
    # =========================================================
    print(
        f"[Visit-EEG distance] Kept {keep_mask.sum()}/{len(gap_tbl)} visits "
        f"in range [{min_days:.2f}, {max_days:.2f}]"
    )

    print("MinAbsGap_days stats:",
          gap_tbl["MinAbsGap_days"].describe().to_dict())
...
def plot_delta_rho_histogram(
    views, vuniq, report_kept,
    near_q=0.333, far_q=0.667,
    ...
    # Spike rate table
    # -----------------------------
    spike_tbl = views["PatientLevelSpikeRates"][
        ["Patient", "MeanSpikeRate_perHour"]
    ].copy()
    ...
    # ensure numeric
    spike_tbl["MeanSpikeRate_perHour"] = pd.to_numeric(
        spike_tbl["MeanSpikeRate_perHour"],
        errors="coerce"
    )
    ...
    def safe_quantile(arr, q):
        arr = np.asarray(arr)
        arr = arr[~np.isnan(arr)]
        return np.nan if arr.size == 0 else np.quantile(arr, q)

    near_days = safe_quantile(gaps, near_q)
    far_days  = safe_quantile(gaps, far_q)

    if len(gaps) == 0:
        raise ValueError("No valid gaps found.")
    ...
    v_near = v_near[v_near["Patient"].isin(base_patients)]
    v_far  = v_far[v_far["Patient"].isin(base_patients)]

    # -----------------------------
    # FIX: handle sz_freqs correctly
    # -----------------------------
    def clean_seizure_freq(x):
        if isinstance(x, (list, np.ndarray, pd.Series)):
            x = pd.to_numeric(pd.Series(x), errors="coerce")
            return np.nanmean(pd.to_numeric(x, errors="coerce"))
        return np.nan

    sz_near = build_patient_seizure_metrics(v_near)
    sz_far  = build_patient_seizure_metrics(v_far)

    # force clean numeric (critical fix)
    sz_near["MeanSzFreq_near"] = pd.to_numeric(sz_near["MeanSzFreq"], errors="coerce")
    sz_far["MeanSzFreq_far"]   = pd.to_numeric(sz_far["MeanSzFreq"], errors="coerce")
    ...
    J = (
        spike_tbl
        .merge(sz_near[["Patient", "MeanSzFreq_near"]], on="Patient", how="inner")
        .merge(sz_far[["Patient", "MeanSzFreq_far"]], on="Patient", how="inner")
    )
    ...
    n = len(J)
    if n < 3:
        raise ValueError(f"Not enough patients ({n}).")
    ...
    # bootstrap (fixed)
    # -----------------------------
    delta = np.zeros(n_boot)

    for i in range(n_boot):
        idx = np.random.randint(0, n, n)

        delta[i] = (
            spearman_rho(x[idx], yN[idx])
    ...
    ax1 = axes[0]
    ax1.hist(gaps, bins=60, alpha=0.6)
    ax1.set_title("Gap distribution")

    ax2 = axes[1]
    ax2.hist(delta, bins=40, alpha=0.7)
