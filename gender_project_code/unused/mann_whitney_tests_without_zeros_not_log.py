...
def calculate_density_jitter(y_vals, width=0.3):
    """
    Calculates a custom horizontal jitter based on linear data density.
    This creates the "bulge" shape where there are more dots, while allowing overlap.
    """
    if len(y_vals) < 3:
        return np.zeros(len(y_vals))
        
    # Microscopic noise prevents crashes if there are too many identical values
    y_noisy = y_vals + np.random.normal(0, 1e-6, size=len(y_vals))
    
    try:
        kde = gaussian_kde(y_noisy)
        density = kde(y_noisy)
...
def plot_scatter_with_stats(df, target_var, title, ylabel, save_path):
    """
    Generates a custom density-scatter plot on a linear scale, over a shaded boxplot.
    """
    # FILTER: Remove values <= 0
    ...
    # 6. Apply Linear Scale & Axis Formatting
...
def main():
    ...
    sz_save_path = os.path.join(data_dir, "B_Seizure_Frequency_Scatter_No_Zero_Log.png")
    ...
    spike_save_path = os.path.join(data_dir, "A_Spike_Rate_Scatter_No_Zero_Log.png")
    plot_scatter_with_stats(
        df=patient_df, 
