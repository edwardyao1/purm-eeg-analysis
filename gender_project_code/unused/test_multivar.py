...
# ==========================================================
# 2. NEGATIVE BINOMIAL REGRESSION STATS (ROBUST MLE METHOD)
# ==========================================================

def get_glm_stats(df, target_col):
    """
    Runs a Multivariable Standalone Negative Binomial Model (NB2).
    This framework uses specialized MLE optimization solvers that handle
    overdispersion and zeros without generating deviance 'nan' errors.
    """
    results = []
    
    # Filter to patients who have complete data
    analysis_df = df.dropna(subset=['nlp_gender', 'canonical_subtype', target_col]).copy()
    analysis_df = analysis_df[analysis_df['canonical_subtype'].isin(['Focal', 'General'])]
    
    if len(analysis_df) == 0:
        return []

    # Define the Multivariable Formula (Female and Focal as Reference groups)
    formula = f"{target_col} ~ C(nlp_gender, Treatment('F')) + C(canonical_subtype, Treatment('Focal'))"
    
    # Fit using standalone Negative Binomial (log-link) optimization
    # This framework handles boundaries/zeros gracefully compared to the GLM IRLS solver
    base_model = smf.negativebinomial(formula, data=analysis_df).fit(disp=0)
    
    # Extract native, mathematically aligned Confidence Intervals (Log scale)
    conf = base_model.conf_int()
    
    var_male = "C(nlp_gender, Treatment('F'))[T.M]"
    var_gen = "C(canonical_subtype, Treatment('Focal'))[T.General]"
    
    # Exponentiate the coefficients and CIs to get Incidence Rate Ratios (IRR)
    irr_male = np.exp(base_model.params[var_male])
    low_male = np.exp(conf.loc[var_male, 0])
    high_male = np.exp(conf.loc[var_male, 1])
    
    irr_gen = np.exp(base_model.params[var_gen])
    low_gen = np.exp(conf.loc[var_gen, 0])
    high_gen = np.exp(conf.loc[var_gen, 1])
    
    # Format Results for Table and Plot
    # --- SEX CATEGORY ---
    females = analysis_df[analysis_df['nlp_gender'] == 'F']
    males = analysis_df[analysis_df['nlp_gender'] == 'M']
    
    results.append({
        'category': 'Sex (Adjusted for Subtype)', 'subgroup': 'Female', 'n': len(females),
        'mean': np.mean(females[target_col]), 'irr': 1.0, 'low': np.nan, 'high': np.nan, 'pval': np.nan, 'is_ref': True
    })
    
    results.append({
        'category': '', 'subgroup': 'Male', 'n': len(males),
        'mean': np.mean(males[target_col]), 'irr': irr_male, 
        'low': low_male, 'high': high_male, 'pval': base_model.pvalues[var_male], 'is_ref': False
    })
    
    # --- SUBTYPE CATEGORY ---
    focals = analysis_df[analysis_df['canonical_subtype'] == 'Focal']
    generals = analysis_df[analysis_df['canonical_subtype'] == 'General']
    
    results.append({
        'category': 'Epilepsy Type (Adjusted for Sex)', 'subgroup': 'Focal', 'n': len(focals),
        'mean': np.mean(focals[target_col]), 'irr': 1.0, 'low': np.nan, 'high': np.nan, 'pval': np.nan, 'is_ref': True
    })
    
    results.append({
        'category': '', 'subgroup': 'General', 'n': len(generals),
        'mean': np.mean(generals[target_col]), 'irr': irr_gen, 
        'low': low_gen, 'high': high_gen, 'pval': base_model.pvalues[var_gen], 'is_ref': False
    })
...
def draw_glm_forest_plot(data, title, save_path, xmin=None, xmax=None, xticks=None):
    if not data: 
        return
    ...
    for y, row in zip(y_pos, data):
        ...
        if row['is_ref']:
            diff_str = f"1.00 (Reference)"
            p_str, ap_str = "-", "-"
        else:
            diff_str = f"{row['irr']:.2f} ({row['low']:.2f} to {row['high']:.2f})"
    ...
    # --- 3. DRAW PLOT ---
    ax_mid.axvline(1.0, color='black', linewidth=1.0, linestyle='--', zorder=1) # Ref Line at 1.0
    ...
    for y, row in zip(y_pos, data):
        if row['is_ref']:
            ...
            ax_mid.plot(row['irr'], y, marker='s', color='black', markersize=9, zorder=3)
            
    # Apply standard linear limits requested by user on layout axis mapping
    ...
    ax_mid.set_xlabel("Incidence Rate Ratio\n<-- Lower Rate in Group   |   Higher Rate in Group -->", labelpad=15, fontsize=11, fontweight='bold')
    fig.suptitle(title, fontsize=16, fontweight='bold', y=1.05)
    
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"-> Saved Corrected Negative Binomial Forest Plot: {save_path}")
...
def main():
    ...
    print("\n======================================================")
    print("=== MULTIVARIABLE NEGATIVE BINOMIAL MODEL ===")
    print("======================================================")

    # 1. Spike Rate Plot (Configured from -1.0 to 2.5)
    spike_data = get_glm_stats(patient_df, "spike_rate_per_hour")
    draw_glm_forest_plot(
        data=spike_data, 
        title="Multivariable Negative Binomial: Factors Predicting Spike Rate", 
        save_path=os.path.join(data_dir, "ForestPlot_GLM_SpikeRate.png"),
        xmin=-1.0, 
        xmax=2.5, 
    ...
    sz_data = get_glm_stats(patient_df, "mean_sz_freq")
    draw_glm_forest_plot(
        data=sz_data, 
        title="Multivariable Negative Binomial: Factors Predicting Seizure Frequency", 
        save_path=os.path.join(data_dir, "ForestPlot_GLM_SzFreq.png"),
        xmin=-2.0, 
        xmax=5.0, 
