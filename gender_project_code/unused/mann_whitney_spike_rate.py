...
def main():
    ...
    # Filter 3: Recording Duration > 0
    before_len = len(df)
    df = df[df["Duration_sec"] > 0].copy()
    track_filter("Filter: Duration > 0", before_len, len(df))

    # Filter 4: Recording Duration <= 4 hours
    before_len = len(df)
    df = df[df["Duration_sec"] <= 4 * 3600].copy()
    track_filter("Filter: Duration <= 4 hours", before_len, len(df))

    # Filter 5: Valid count_0_46 (Spike count exists)
    before_len = len(df)
    df = df[df["count_0_46"].notna()].copy()
    track_filter("Filter: Valid count_0_46", before_len, len(df))
    ...
    # We include the clinical columns in the groupby to retain them, 
    # since they are constant per patient.
    ...
    # Calculate spike rate per hour for each patient
