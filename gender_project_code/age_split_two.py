...
# Modeled by sex, epilepsy type, and age groups (18-50, 51+) 
...
def print_and_save_requested_stats(patient_df, valid_sessions, vuniq, save_dir, n_boot=2000):
    ...
    n_lt_51 = len(patient_df[patient_df['age_group'] == '18-50'])
    n_ge_51 = len(patient_df[patient_df['age_group'] == '51+'])
    ...
    table_data = [
        ...
        {"Category": "Age Group", "Metric": "18-50 years N (%)", "Value": f"{n_lt_51:,} ({(n_lt_51/n_patients)*100:.1f}%)", "Bootstrapped 95% CI": ""},
    ...
    # =========================================================================
    # 4. Save as Pretty Publication PNG Image (FIXED LAYOUT & TEXT CLIP)
    # =========================================================================
...
def run_analysis(patient_df, outcome_col, outcome_label):
    ...
    # Note: pd.get_dummies(drop_first=True) on categorical ['18-50', '51+'] drops '18-50'
    formula_vars = ['nlp_gender_M', 'canonical_subtype_General', 'age_group_51+']
    ...
    # Section 3: Age Group
    age1_n = len(patient_df[patient_df['age_group']=='18-50'])
    rows.append(dict(section="Age Group", label="18-50 years", n=age1_n, is_reference=True, diff=0, lo=0, hi=0, p=None))
    
    a2_res = coeffs['age_group_51+']
    rows.append(dict(section="Age Group", label="51+ years", n=len(patient_df[patient_df['age_group']=='51+']), is_reference=False, diff=a2_res[0], lo=a2_res[1], hi=a2_res[2], p=a2_res[3]))
...
def print_logit_results(df, outcome_col, title):
    ...
    reg_df["age_group_51+"] = (reg_df["age_group"] == "51+").astype(float)

    X = reg_df[["nlp_gender_M", "canonical_subtype_General", "age_group_51+"]].astype(float)
    X = sm.add_constant(X)
...
def main():
    ...
    if not vuniq.empty:
        # Track if frequency was documented prior to 0.0 imputation
    ...
    # --------------------------------------------------------
    # CREATE CATEGORICAL AGE GROUP (18-50 as reference)
    # --------------------------------------------------------
    bins = [18, 51, np.inf] # [18, 51) evaluates exactly to 18.0 up to 50.999
    labels = ['18-50', '51+']
    ...
    forest_plot(
        ...
        os.path.join(save_dir, "forest_spike_rate_boot_binary_agegroup_51.png")
    )
    ...
    forest_plot(
        ...
        "Coefficient (Log-Odds)", 
        os.path.join(save_dir, "forest_seizure_freq_boot_binary_agegroup_51.png")
    )
