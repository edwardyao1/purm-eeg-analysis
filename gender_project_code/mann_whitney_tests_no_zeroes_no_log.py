...
def plot_scatter_with_stats(df, target_var, title, ylabel, save_path):
    ...
    ax = sns.boxplot(
        ...
        order=["Female", "Male"], showfliers=False, width=0.4, 
    ...
    for i, group in enumerate(["Female", "Male"]):
        ...
        if len(y_vals) > 0:
            ...
            x_vals = i + jitter
            ax.scatter(
                x_vals, y_vals, color=custom_palette[group], s=20,          
...
def plot_stratified_scatter_with_stats(df, target_var, title, ylabel, save_path):
    """
    Generates a 1x2 side-by-side plot stratifying the data by Epilepsy Subtype (Focal vs General).
    """
    plot_df = df.copy()
    plot_df['Sex'] = plot_df['nlp_gender'].map({'F': 'Female', 'M': 'Male'})
    plot_df[target_var] = plot_df[target_var].clip(lower=0) 
    
    subtypes = ["Focal", "General"]
    ...
    for ax, subtype in zip(axes, subtypes):
        ...
        sns.boxplot(
            x="Sex", y=target_var, data=sub_df, 
            order=["Female", "Male"], showfliers=False, width=0.4, 
            boxprops=dict(facecolor="lightgray", edgecolor="black", linewidth=2.5, alpha=0.7),
            medianprops=dict(color="black", linewidth=3.5),
            whiskerprops=dict(color="black", linewidth=2.5),
        ...
        if ax == axes[0]:
            ax.set_ylabel(f"{ylabel}", fontsize=14, fontweight='bold')
        else:
            ax.set_ylabel("")
...
def main():
    ...
    # -------------------------------------------------------------------
    # APPLIED THE STRICT ALLOWLIST HERE
    # -------------------------------------------------------------------
    before = len(df)
    ...
    patient_df = df.groupby(
        ["Patient", "nlp_gender", "epilepsy_type"], 
        dropna=False
    ).agg(
        total_spikes=("count_0_46", "sum"),
        total_duration=("Duration_sec", "sum"),
        mean_sz_freq=("sz_freq_numeric", "mean"),
        canonical_subtype=("canonical_subtype", "first")
    ).reset_index()
    ...
    spike_save_path = os.path.join(save_dir, "Figure1_Overall_Spike_Rate_No_Zero.png")
    plot_scatter_with_stats(
        df=patient_df, 
        target_var="spike_rate_per_hour", 
        title="Figure 1\nOverall Men vs Women: Spike Rate", 
        ylabel="Spikes per Hour", 
        save_path=spike_save_path
    )
    
    sz_save_path = os.path.join(save_dir, "Figure2_Overall_Seizure_Frequency_No_Zero.png")
    plot_scatter_with_stats(
        df=patient_df, 
        target_var="mean_sz_freq", 
        title="Figure 2\nOverall Men vs Women: Seizure Frequency", 
        ylabel="Average Seizure Frequency", 
        save_path=sz_save_path
    )

    fig3_save_path = os.path.join(save_dir, "Figure3_Spike_Rate_Stratified_No_Zero.png")
    plot_stratified_scatter_with_stats(
        df=patient_df, 
        target_var="spike_rate_per_hour", 
        title="Figure 3: Spike Rate Distribution by Sex Stratified by Epilepsy Subtype", 
        ylabel="Spikes per Hour", 
        save_path=fig3_save_path
    )

    fig4_save_path = os.path.join(save_dir, "Figure4_Seizure_Frequency_Stratified_No_Zero.png")
    plot_stratified_scatter_with_stats(
        df=patient_df, 
        target_var="mean_sz_freq", 
        title="Figure 4: Seizure Frequency Distribution by Sex Stratified by Epilepsy Subtype", 
        ylabel="Average Seizure Frequency", 
        save_path=fig4_save_path
    )
