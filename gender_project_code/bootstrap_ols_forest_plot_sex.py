...
# ==========================================================
# BOOTSTRAP & OLS HELPERS (Updated to Regress only on Sex)
# ==========================================================
def bootstrap_regression_coeffs(df, outcome_col, formula_vars, n_boot=5000):
    ...
    # Get dummies only for the outcome and gender (ignoring epilepsy_type)
    reg_df = pd.get_dummies(df[[outcome_col, "nlp_gender"]], drop_first=True).astype(float)
...
def run_analysis(patient_df, outcome_col, outcome_label):
    ...
    # Formula only includes Male dummy variable
    formula_vars = ['nlp_gender_M']
...
def forest_plot(rows, title, x_lim, x_ticks, x_label, out_path):
    # Reduced figure height for single category plotting
    ...
    axis_y = min(ypos.values()) - 1.5
    ax.plot([map_x(0), map_x(0)], [axis_y, top], ls="--", color="gray")
...
def print_ols_results(df, outcome_col, title):
    ...
    # We keep only gender and outcome for the regression
    reg_df = df[["nlp_gender", outcome_col]].copy()
    ...
    print("\nOutcome summary:")
    print(reg_df[outcome_col].describe())
    ...
    X = reg_df[["nlp_gender_M"]].astype(float)
    X = sm.add_constant(X)
...
def main():
    ...
    # 6. RUN ANALYSIS (Bootstrapping restricted to Sex only)
    ...
    forest_plot(
        rows_spike,
        "Bootstrapped OLS: Spike Rate (per hour) ~ Sex",
        (-0.2, 0.2),
        [-0.2, -0.1, 0, 0.1, 0.2],
        "Coefficient", 
        os.path.join(save_dir, "forest_spike_rate_boot_ols_sex.png")
    )

    forest_plot(
        rows_sz,
        "Bootstrapped OLS: Seizure Frequency (per month) ~ Sex",
        (-0.2, 0.2),
        [-0.2, -0.1, 0, 0.1, 0.2],
        "Coefficient", 
        os.path.join(save_dir, "forest_seizure_freq_boot_ols_sex.png")
    )
    print("Done plotting.")

    # 8. STANDARD OLS REGRESSION RESULTS
    spike_model = print_ols_results(patient_df, "spike_rate_per_hour", "OLS REGRESSION: SPIKE RATE ~ SEX")
    sz_model = print_ols_results(patient_df, "mean_sz_freq", "OLS REGRESSION: SEIZURE FREQUENCY ~ SEX")
