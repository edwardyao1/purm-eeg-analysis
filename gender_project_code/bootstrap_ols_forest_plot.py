...
def print_ols_results(df, outcome_col, title):
    ...
    # --- ADDED: Print the raw outcome summary before log transformation ---
    print("\nOutcome summary:")
    ...
    # --- ADDED: Print the dtypes for the independent and dependent variables ---
    print("\nX dtypes:")
    print(X.dtypes)
    print("\ny dtype:")
    print(y.dtype)
    ...
    # --- ADDED: Print Confidence Intervals and Exact Coefficients ---
    print("\n95% CI")
...
def main():
    ...
    forest_plot(
        rows_spike,
        "Bootstrapped OLS Coefficients: Spike Rate (per hour) ~ Sex + Epilepsy Type",
        (-0.2, 0.6),
        [-0.2, 0, 0.2, 0.4, 0.6],
        "Coefficient", 
        os.path.join(save_dir, "forest_spike_rate_boot_ols.png")
    )
    
    forest_plot(
        rows_sz,
        "Bootstrapped OLS Coefficients: Seizure Frequency (per month) ~ Sex + Epilepsy Type",
        (-0.2, 0.2),
        [-0.2, -0.1, 0.0, 0.1, 0.2],
        "Coefficient", 
        os.path.join(save_dir, "forest_seizure_freq_boot_ols.png")
    )

    print("\nBootstrap Pipeline Complete. All outputs saved to:", save_dir)
